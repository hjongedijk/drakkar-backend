  ...
  async function updateProgress(chunkLength = 0, force = false) {
    ...
    progressWrite = progressWrite
      ...
      .then(async () => {
        ...
        await safeUpdateDownload(input.downloadId, progressData(snapshot));
      })
      .catch((error) => {
...
export async function prepareNzbDocumentForStreaming(input: {
  ...
  if (providers.length === 0) {
    await safeUpdateDownload(input.downloadId, { status: "waiting_for_provider", error: "No enabled Usenet providers configured" });
  ...
  try {
    ...
    await safeUpdateDownload(input.downloadId, { status: "failed", error: message, speedBytesSec: 0 });

...
import { fetchCalendarMediaInfo, fetchSeasonEpisodes, fetchSeriesStructure } from "../services/metadataService.js";
import { getSettings } from "../services/settings/settingsStore.js";
import { hydrateLegacyMediaFields, hydrateLegacyRequestFields } from "../services/media-library/normalizedMedia.js";
...
const CALENDAR_LIBRARY_RELATION_SELECT = {
  movie: {
    select: {
      tmdbId: true,
      imdbId: true,
...
const CALENDAR_REQUEST_RELATION_SELECT = {
  movie: {
    select: {
      tmdbId: true,
      imdbId: true,
...
export async function fetchReleaseCalendar(month?: string, options?: { compact?: boolean }): Promise<ReleaseCalendarResponse> {
  const range = monthRange(month);
  const cacheKey = `release-calendar:v6:${range.month}`;
  const cached = await redis.get(cacheKey).catch(() => null);
  if (cached) {
    const response = JSON.parse(cached) as ReleaseCalendarResponse;
    return options?.compact ? compactCalendarResponse(response) : response;
  }
  ...
  const [libraryItems, requests] = await Promise.all([
    prisma.mediaLibraryItem.findMany({
      ...
      select: {
        ...
        season: true,
        episode: true,
        ...CALENDAR_LIBRARY_RELATION_SELECT
      },
      orderBy: { title: "asc" }
    }),
    prisma.mediaRequest.findMany({
      ...
      select: {
        ...
        tvdbId: true,
        imdbId: true,
        ...CALENDAR_REQUEST_RELATION_SELECT
      },
      orderBy: { title: "asc" }
    })
  ]);
  ...
  const hydratedLibraryItems = libraryItems.map((entry) => hydrateLegacyMediaFields(entry) as typeof entry & {
    posterUrl?: string | null;
    backdropUrl?: string | null;
  ...
  for (const item of hydratedLibraryItems) {
  ...
  for (const request of requests.map((entry) => hydrateLegacyRequestFields(entry))) {
  ...
  const movieEntries = (await mapWithConcurrency<TrackedMovie, ReleaseCalendarEntry | null>([...movieMap.values()], CALENDAR_FETCH_CONCURRENCY, async (trackedMovie) => {
    ...
    return {
      ...
      releaseDate,
      overview: options?.compact ? undefined : info?.overview,
      mediaType: "movie",
    ...
    for (const season of seasons) {
      for (const episode of season.episodes) {
        ...
        const entry: ReleaseCalendarEntry = {
          ...
          releaseDate: airDate,
          overview: options?.compact ? undefined : episode.overview,
          mediaType: "tv",
...
function compactCalendarResponse(response: ReleaseCalendarResponse): ReleaseCalendarResponse {
  return {
    ...response,

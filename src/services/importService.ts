...
import { detectArchive, isJunkFile, isMediaFile } from "../services/extract/detect.js";
...
import { createLibraryEntryForImport, resolveImportMedia } from "../services/symlinkService.js";
import { filenameFromSubject } from "../services/usenet/filename.js";
import { listMountedFiles } from "../services/mountedNzbService.js";
...
const IMPORT_REQUEST_RELATION_SELECT = {
  movie: {
    select: {
      tmdbId: true,
      imdbId: true,
...
async function requestMetadata(requestId?: string): Promise<Partial<ImportMedia>> {
  ...
  const request = await prisma.mediaRequest.findUnique({
    where: { id: requestId },
    include: IMPORT_REQUEST_RELATION_SELECT
...
function buildImportIdentity(media: {
  mediaType: string;
  title: string;
  year?: number | null;
  season?: number | null;
...
async function findExistingImportByIdentity(media: {
  mediaType: string;
  title: string;
  year?: number | null;
  season?: number | null;
  ...
  const identityKey = buildImportIdentity(media);
  return prisma.importItem.findUnique({ where: { identityKey }, include: { symlinks: true } });
}
...
export async function reconcileRequestStatusAfterImport(requestId?: string, downloadId?: string | null) {
  ...
  if (request.mediaType !== "tv") {
    ...
    const { markRequestAvailable } = await import("../services/requests/sync/service.js");
    await markRequestAvailable(requestId).catch(() => undefined);
    return;
  }

  const [{ getRequestMonitor }, activeDownload] = await Promise.all([
    import("../services/requests/sync/service.js"),
    request.downloadId
...
export async function importCompletedPath(input: { sourcePath: string; downloadId?: string; requestId?: string }) {
  ...
  for (const sourcePath of mediaFiles) {
    ...
    const identityKey = buildImportIdentity(inferred);
    const item = await prisma.importItem.upsert({
      where: { identityKey },
      update: {
...
export async function makeMountedDownloadAvailable(input: { downloadId: string; requestId?: string }) {
  ...
  for (const selected of selectedFiles) {
    ...
    const identityKey = buildImportIdentity({
      mediaType: inferred.mediaType,
...
export async function repairSuspiciousImports(options: { limit?: number } = {}) {
  ...
  for (const item of imports.slice(0, limit)) {
    ...
    const updated = await prisma.importItem.update({
      where: { id: item.id },
      data: {
        identityKey: buildImportIdentity({
          mediaType: inferred.mediaType,
          title: nextTitle,
...
async function findWorkingImportByIdentity(media: Partial<ImportMedia>) {
  ...
  const direct = await findExistingImportByIdentity({
    mediaType: media.mediaType,
    title: media.title,
    year: media.year,
    season: media.season,
  ...
  if (direct && direct.symlinks.some((link) => link.status !== "broken")) {
    if (!direct.completedPath.startsWith("/mounted/")) return direct;
    const fileId = mountedFileIdFromPath(direct.completedPath);
    if (fileId) {
      const nzbFile = await prisma.nzbFile.findUnique({ where: { id: fileId }, select: { subject: true } });
      if (nzbFile?.subject && !/\.par2\b/i.test(nzbFile.subject)) return direct;
    }
  }
